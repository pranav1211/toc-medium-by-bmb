...
(function () {
  ...
  // ── Resolve the base URL for anchor links ─────────────────────────────────────
  // On an edit page, location.href looks like https://medium.com/p/<id>/edit
  // Pasting <a href="#abc"> into the editor would resolve to .../edit#abc, which
  // is broken for published readers. Use the canonical link when available, else
  // strip the trailing /edit segment.
  function stripEdit(u) {
    return (u || '').split('#')[0].split('?')[0].replace(/\/edit\/?$/, '');
  }

  function getArticleBaseURL() {
    // 1. og:url — on Medium this is usually the published canonical URL
    //    (e.g. medium.com/@author/<slug>-<id>) even when viewing the editor.
    const og = document.querySelector('meta[property="og:url"]');
    if (og && og.content) {
      const u = stripEdit(og.content);
      if (u) { console.log(TAG, 'baseURL via og:url ->', u); return u; }
    }

    // 2. <link rel="canonical">
    const canonical = document.querySelector('link[rel="canonical"]');
    if (canonical && canonical.href) {
      const u = stripEdit(canonical.href);
      if (u) { console.log(TAG, 'baseURL via canonical ->', u); return u; }
    }

    // 3. Twitter card URL
    const tw = document.querySelector('meta[name="twitter:url"], meta[property="twitter:url"]');
    if (tw && tw.content) {
      const u = stripEdit(tw.content);
      if (u) { console.log(TAG, 'baseURL via twitter:url ->', u); return u; }
    }

    // 4. Fallback: strip /edit from current URL — yields .../p/<id>
    //    which Medium will redirect to the canonical published article.
    const u = stripEdit(location.href);
    console.log(TAG, 'baseURL via location fallback ->', u);
    return u;
  }
  ...
  function buildCopyHTML_Paragraphs(headings, settings) {
    ...
    const minLevel = Math.min(...headings.map(h => h.level));
    const baseURL = getArticleBaseURL();
    const tocParts = [];
    ...
    lines.forEach((line, idx) => {
      ...
      let entry = `${indent}${prefix}<a href="${escHtml(baseURL)}#${line.id}">${linkText}</a>`;
      const isSub = rel > 0;
  ...
  function buildCopyHTML_NativeList(headings, settings) {
    ...
    groups.forEach(group => {
      ...
      let mainEntry = `<a href="${escHtml(baseURL)}#${line.id}">${escHtml(heading.text)}</a>`;
      ...
      group.subs.forEach(sub => {
        ...
        let subEntry = `${indent}${prefix}<a href="${escHtml(baseURL)}#${sub.line.id}">${escHtml(sub.heading.text)}</a>`;
  ...
  // ── Floating launcher button (auto-shows on edit pages) ──────────────────────
  let launcherEl = null;

  function showLauncher() {
    if (!isEditPage()) return;
    if (document.getElementById('bmb-toc-launcher')) return;
    if (panelEl) return; // don't show when panel is open

    const btn = document.createElement('button');
    btn.id = 'bmb-toc-launcher';
    btn.title = 'Open Medium TOC (Alt+T)';
    btn.style.cssText = `
      position: fixed;
      bottom: 24px;
      right: 24px;
      ...
      color: #e8ff6b;
      cursor: pointer;
      box-shadow: 0 6px 20px rgba(0,0,0,0.25), 0 2px 6px rgba(0,0,0,0.1);
      display: flex;
      align-items: center;
    ...
    `;
    btn.innerHTML = `<img src="${chrome.runtime.getURL('icons/icon128.png')}"
      style="width:28px;height:28px;border-radius:6px;pointer-events:none;" />`;

    btn.addEventListener('mouseenter', () => {
      btn.style.transform = 'scale(1.08)';
      btn.style.boxShadow = '0 8px 24px rgba(0,0,0,0.3), 0 3px 8px rgba(0,0,0,0.15)';
    });
    btn.addEventListener('mouseleave', () => {
      btn.style.transform = 'scale(1)';
      btn.style.boxShadow = '0 6px 20px rgba(0,0,0,0.25), 0 2px 6px rgba(0,0,0,0.1)';
    });
    ...
    launcherEl = btn;
    console.log(TAG, 'Launcher shown');
  }

  function hideLauncher() {
    document.getElementById('bmb-toc-launcher')?.remove();
    launcherEl = null;
  }

  // Inject the launcher on edit pages once the DOM is ready.
  function maybeShowLauncherOnEditPage() {
    if (!isEditPage()) { hideLauncher(); return; }
    if (document.body) {
      showLauncher();
    } else {
  ...
  maybeShowLauncherOnEditPage();

  // Medium is an SPA — watch for URL changes so the launcher appears/disappears
  // when navigating into/out of /edit without a full reload.
